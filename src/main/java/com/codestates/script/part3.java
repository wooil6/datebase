...
public class part3 {
  ...
  */
  ...
  /*
  ----------------------------------------------------------------------------------------------
     C   TODO: Q 3-2-2. user의 name과 email 그리고 그 user가 속한 role name(컬럼명: roleName)을 찾기 위한 SQL을 작성해주세요.
  ...
  */
  public static final String PART3_2_5 = "SELECT content.title FROM user JOIN content ON user.id = content.userId WHERE user.name = 'minsanggu' ";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 3-2-6. minsanggu이 작성한 content의 category name을 찾기위한 SQL을 작성해주세요.
  */
  public static final String PART3_2_6 = "SELECT category.name " +
          "FROM category JOIN content_category ON category.id = content_category.categoryId " +
  ...
  */
  public static final String PART3_2_7 = "SELECT content.title, content.body, content.created_at " +
          "FROM content JOIN content_category ON content_category.contentId = content.id " +
  ...
  */
  public static final String PART3_2_8 = "SELECT content.title, content.body, content.created_at, user.name " +
          "FROM user JOIN content ON user.id = content.userId " +
  ...
  */
  public static final String PART3_2_9 = "SELECT count(content.id) AS ContentCount " +
          "FROM content JOIN user ON user.id = content.userId " +
  ...
  */
  public static final String PART3_2_10 = "SELECT user.name AS name, COUNT(content.id) AS ContentCount " +
          "FROM user LEFT JOIN content ON user.id = content.userid " +
          "GROUP BY user.name ";
